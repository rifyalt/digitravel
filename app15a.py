...
with st.sidebar:
    ...
    st.markdown("---")
    mode = st.radio("Pilih sumber file utama:", ["Google Drive Folder", "Upload Manual"])

# ===== FILE AMBIL DARI GDRIVE / UPLOAD =====
data_utama_files = []
if mode == "Google Drive Folder":
    folder_url = st.text_input("🔗 Masukkan Link Folder Google Drive (share anyone with link)")
    if st.button("Ambil File dari GDrive"):
        if folder_url.strip():
            try:
                ...
                if folder_id:
                    ...
                    for fname in os.listdir("data_temp"):
                        if fname.endswith(".xlsx"):
                            data_utama_files.append(os.path.join("data_temp", fname))
                    st.success(f"✅ {len(data_utama_files)} file berhasil diambil dari Google Drive.")
                else:
                    st.error("❌ Link folder tidak valid.")
            except Exception as e:
                st.error(f"❌ Gagal ambil file dari GDrive: {e}")
else:
...
# ===== FILE TAMBAHAN (tetap sama) =====
col1, col2 = st.columns(2)
additional_files = []

with col1:
    file_data1 = st.file_uploader("📄 File Data 1 (Optional single)", type=["xlsx"], key="data1")
    if file_data1:
        additional_files.append(file_data1)

    multi_files_2 = st.file_uploader("📂 File Data 2 (Multi Upload, Optional)", type=["xlsx"], accept_multiple_files=True, key="data2")
    if multi_files_2:
        additional_files.extend(multi_files_2)

with col2:
    multi_files_3 = st.file_uploader("📂 File Data 3 (Multi Upload, Optional)", type=["xlsx"], accept_multiple_files=True, key="data3")
    if multi_files_3:
        additional_files.extend(multi_files_3)

    multi_files_4 = st.file_uploader("📂 File Data 4 (Multi Upload, Optional)", type=["xlsx"], accept_multiple_files=True, key="data4")
    if multi_files_4:
        additional_files.extend(multi_files_4)

# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
# Bagian join/filter/analisis TETAP sama dengan kode asli Anda
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
